...
class Beanstalkd {
    ...
    public static function o($conn = '', $option = []) {
        ...
        if (empty($container[$conn]) || !is_object($container[$conn])) {
            $container[$conn] = new Beanstalkd($conn, $option);
        }
	 ...
	 * @return void
	 */
	public function __construct($conn, $option = []) {
        $CI = & get_instance();
		$CI->load->config('service/beanstalkd');
        $beanstalkd_config = $CI->config->item('beanstalkd');
		$this->_config = $beanstalkd_config[$conn];
	...
	public function connect() {
		...
		$function = $this->_config['pconnect'] ? 'pfsockopen' : 'fsockopen';
