...
class PHPMailer {
 ...
 /**
  ...
  ////////////////////////////////////////////////
  // Added by delongwu@sohu-inc.com
  // chinese filepath or filename may use this, if these variables are set,
  // we will user iconv to change encoding.
  ////////////////////////////////////////////////
  private $fromEncoding = '', $toEncoding = '';
  // the reply the server sent to us for DATA
  private $data_command_rply;
   ...
   */
  public function convertFilePathEncoding($fromEncoding = '', $toEncoding = '') {
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    ...
    if ($kind != 'Reply-To') {
	 // modify by delongwu@sohu-inc.com since we nedd to deliver the same address.
 ...
 */
  ...
  protected function SmtpSend($header, $body) {
    ...
    // get the data response. added by delongwu@sohu-inc.com
    $this->data_command_rply = $this->smtp->getDataCommandRply();
   ...
   * added by delongwu@sohu-inc.com 
   */
  public function getDataCommandRply(){
  	return $this->data_command_rply;
  }
  ...
  */
  ...
  public function CreateHeader() {
    ...
    // put x-smtpapi the last line of this String, modify by delongwu@sohu-inc.com
     $smtpApiExt = "";
    // Add custom headers
    for($index = 0; $index < count($this->CustomHeader); $index++) {
	    if ($this->CustomHeader[$index][0] == 'X-SMTPAPI') {
	    	$smtpApiExt = $this->CustomHeader[$index][1];
	        continue;
	    }
    ...
    if ($smtpApiExt != '') {
    	...
    	$result = rtrim($result, $this->LE);
    	$result .= $this->LE;
    	$result .= $this->HeaderLine('X-SMTPAPI', $smtpApiExt);
    	$result .= $this->LE;
    }
...
//   }
  ...
  /**
   * replace the php base since basename(chinesepath) can not return correct.
   *  
   * @param String $filename the filename
   * @return String
   * @author delongwu@sohu-inc.com
   */
   ...
   */
  public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
  	// some chinese attachment falied, so modify this.
  	// @author delongwu@sohu-inc.com
  	try {
  		$filename = $this->getBaseName($path);
  		if ($this->fromEncoding != '' && $this->toEncoding != '') {
  			$path = iconv($this->fromEncoding, $this->toEncoding, $path);
  			//$filename= "=?". $this->fromEncoding. "?B?" . base64_encode($filename) . "?=";
  		}
  ...
  */
  ...
  public function EncodeQ($str, $position = 'text') {
  	// TODO: Change by delongwu@sohu-inc.com??
  	$pattern = '';
  ...
  public function AddStringAttachment($string, $filename, $encoding = 'base64', $type = 'application/octet-stream') {
    ...
    $this->attachment[] = array(
      0 => $string,
      1 => $filename,
      2 => $this->getBaseName($filename), // modified by delongwu@sohu-inc.com, Since chinese basename not correct.
      3 => $encoding,
      4 => $type,
  ...
  public function AttachmentExists() {
    foreach($this->attachment as $attachment) {
      if ($attachment[6] == 'attachmentModel') {
        return true;
      }
    }
    return false;
