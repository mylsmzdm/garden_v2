...
require_once __DIR__.'/WebUtil.php';
...
class ZmopClient {
    ...
    function __construct($gatewayUrl = '', $appId = '', $charset = 'UTF-8', $privateKeyContent, $zhiMaPublicKeyContent) {
        $this->gatewayUrl = $gatewayUrl;
        $this->appId = $appId;
        $this->privateKeyContent = $privateKeyContent;
        $this->zhiMaPublicKeyContent = $zhiMaPublicKeyContent;
    ...
    public function execute($request) {
        ...
        $sysParams[$this->signParam] = RSAUtil::sign($apiParamsQuery, $this->privateKeyContent);


        //加密,只对业务参数进行加密
        $encryptedApiParams[$this->bizParam] = RSAUtil::rsaEncrypt($apiParamsQuery, $this->zhiMaPublicKeyContent);

        //如果是文件上传，那么需要把文件参数也放到POST数组里面
        $files = $request->getFileParas();
        if ($files != null) {
            foreach ($files as $k => $v) {
                $encryptedApiParams[$k] = "@" . $v;
            }
        }
        ...
        //发起HTTP请求
        try {
            $resp = WebUtil::curl($requestUrl, $encryptedApiParams, $this->charset);#echo $resp;exit;
        } catch (Exception $e) {
        ...
        if ("json" == $this->format) {
            ...
            if ($encrypted) {
                $biz_response = RSAUtil::rsaDecrypt($biz_response, $this->privateKeyContent);
            }
        ...
        if ($bizResponseSign != null) {
            $verifyResult = RSAUtil::verify($biz_response, $bizResponseSign, $this->zhiMaPublicKeyContent);
    ...
    public function generatePageRedirectInvokeUrl($request) {
        ...
        $systemParams[$this->signParam] = RSAUtil::sign($apiParamsQuery, $this->privateKeyContent);
        ...
        $bizParam = RSAUtil::rsaEncrypt($apiParamsQuery, $this->zhiMaPublicKeyContent);
    ...
    public function decryptAndVerifySign($encryptedResponse, $sign) {
        $decryptedResponse = RSAUtil::rsaDecrypt($encryptedResponse, $this->privateKeyContent);
        $success = RSAUtil::verify($decryptedResponse, $sign, $this->zhiMaPublicKeyContent);
        if (success) {
    ...
    public function generateSign($request) {
        ...
        return RSAUtil::sign($paramStr, $this->privateKeyContent);
    }
    ...
    public function generateEncryptedParam($request) {
        $paramStr = $this->getBizParamStr($request);
        return RSAUtil::rsaEncrypt($paramStr, $this->zhiMaPublicKeyContent);
    }
