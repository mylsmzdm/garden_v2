...
class Saetoauthv2 {
	...
	function http($url, $method, $postfields = NULL, $headers = array()) {
		...
		if ( !empty($this->remote_ip) ) {
			...
			if ( !defined('SAE_ACCESSKEY') ) {
				$headers[] = "API-RemoteIP: 218.245.0.171";//$headers[] = "API-RemoteIP: " . $_SERVER['REMOTE_ADDR'];//定时作业中不能得到REMOTE_ADDR
			}
		}
...
class SaeTClientV2
{
	 ...
	 * 
	 * @access public
	 * @param mixed $client_id 微博开放平台应用APP KEY
	 * @param mixed $client_secret 微博开放平台应用APP SECRET
	...
	{
		$this->oauth = new Saetoauthv2($param);
	}
